...
/* harmony import */ var _modules_culc__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/culc */ "./src/js/modules/culc.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_2__["default"])('[name = "message"]'); // showMoreStyles('.button-styles', '.styles-2');  // заком., используем БД

  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_11__["default"])('.button-styles', '#styles .row');
  Object(_modules_culc__WEBPACK_IMPORTED_MODULE_3__["default"])('#size', '#material', '#options', '.promocode', '.calc-price');
  Object(_modules_filter__WEBPACK_IMPORTED_MODULE_5__["default"])();
  Object(_modules_pictureSize__WEBPACK_IMPORTED_MODULE_9__["default"])('.sizes-block'); // accordion('.accordion-heading ', '.accordion-block');  // css-animation

  Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_0__["default"])('.accordion-heading'); // using js
...
var accordion = function accordion(triggersSelector) {
  // trigger - обьект (текст) по которому кликаем, items - обьект, который появляется
...
var checkTextInputs = function checkTextInputs(selector) {
  ...
  textInputs.forEach(function (input) {
    input.addEventListener('keypress', function (e) {
      // keypress - нажатие на определенную клавишу
      if (e.key.match(/[^а-яё 0-9]/ig)) {
        // e.key - значение клавиши, проверяем на соответствие кирилице
        e.preventDefault();
      }
    });
  });
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var culc = function culc(size, material, options, promocode, result) {
...
var drop = function drop() {
  ...
  var fileInputs = document.querySelectorAll('[name="upload"]'); // выбираем input-ы для загрузки файла
  ...
  function highLight(item) {
    // выделяем область куда можно перетягнуть файл
  ...
  ['dragenter', 'dragover'].forEach(function (eventName) {
    fileInputs.forEach(function (input) {
      ...
      }, false); // выделяем инпут при событиях  
    });
  });
  ['dragleave', 'drop'].forEach(function (eventName) {
    ...
    fileInputs.forEach(function (input) {
      ...
      }, false); // на каждый инпут навешываем кождое событие и убираем стандарное поведение браузера с помощью функции 
    });
  });
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      var name = arr[0].substring(0, 6) + dots + arr[1]; // обрезаем длинное имья
...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (e) {
    ...
    if (target && target.tagName == 'LI') {
      //  поддерживает ли событие "клик" и это ЛИ (обьязательно большими)
...
var forms = function forms() {
  // state, как аргумент - modalState (данные с формы), актуально только для калькулятора
  ...
  upload.forEach(function (item) {
    // получаем имья загружаемого файла
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      statusMessage.classList.add('status'); // item.appendChild(statusMessage);                // добавляем сообщение в конец формы // заком.если нужно отобразить картинку вместо формы (см.ниже)

      item.parentNode.appendChild(statusMessage);
      item.classList.add('animated', 'fadeOutUp'); //если нужно отобразить картинку вместо формы, делаем форму прозрачной

      setTimeout(function () {
        //если нужно отобразить картинку вместо формы, убираем форму (input) со страницы
        item.style.display = 'none';
      }, 400);
      ...
      var api; // динамический путь, куда будем отправлять данные
      ...
      console.log(api); // postData('assets/server.php', formData)
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __',
        // можно создать вручную, как в данному случае, или в отдельном json файле
    i = 0,
        def = matrix.replace(/\D/g, ''),
        val = this.value.replace(/\D/g, '');

    if (def.length >= val.length) {
      // чтобы нельзя было удалить +7
      val = def;
    }

    this.value = matrix.replace(/./g, function (a) {
      // this.value - значение, которое ввел пользователь, a - это каждый символ
    ...
    if (event.type === 'blur') {
      // blur - элемент теряет фокус (выйти из инпута)
      if (this.value.length == 2) {
        // если ничего не ввели, т.е осталось два символа +7
        this.value = '';
      } else {
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        // data-modal - data-атрибут для всех модальных окон
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        document.body.style.marginRight = "".concat(scroll, "px"); // чтобы не прыгала страница про открывании/закрывании модального окна
  ...
  function showModalByTime(selector, time) {
    // чтобы мод.окно появлялось со временем
    setTimeout(function () {
      var display;
      document.querySelectorAll('[data-modal]').forEach(function (item) {
        // проверяем открыто какое-либо модальное окно или нет
  ...
  function calcScroll() {
    // посчитать сколько места занимает правый скрол, чтобы потом не "прыгала" картинка при открытии/закрытии модального окна
    ...
    var scrollWidth = div.offsetWidth - div.clientWidth; // полная ширина минус главный контент в который НЕ включается прокрутка
...
var pictureSize = function pictureSize(imgSelector) {
  // imgSelector - селектор, по которому обьеденяются все изображения / блоки
  var blocks = document.querySelectorAll(imgSelector);

  function showImg(block) {
    // block - блок (диф), где находится изображение
    ...
    block.querySelectorAll('p:not(.sizes-hit)').forEach(function (p) {
      // скрываем параграфы (надписи), кроме одно с соответств. классом
...
var scrolling = function scrolling(upSelector) {
  ...
  }); //============ Плавный скролл с window.requestAnimationFrame()  ===============

  var links = document.querySelectorAll('[href^="#"]'),
      // ищем все ссылки которые начинаются (^ - начало строки) с #
  speed = 0.3;
  ...
  }); //=================== Плавный скролл, только с js ===============
  // const element = document.documentElement,       // element or body - зависит от браузера
  //     body = document.body;
  // const calcScroll = () => {
  //     upElem.addEventListener('click', function (e) {
  //         let scrollTop = Math.round(body.scrollTop || element.scrollTop);
  //         if (this.hash !== '') {     // hash - когда кликнуть на ссылку в рамках одной страницы в командной строке появится хеш, напр.: http://test/#up
  //             e.preventDefault();
  //             // let hashElement = document.getElementById(this.hash.substring(1)),       // от хеша получим строчку без #, т.е. только up
  //             let hashElement = document.querySelector(this.hash),            // тоже самое, что и выше
  //                 hashElementTop = 0;
  //             while (hashElement.offsetParent) {              // проверяем сколько пикселей проскролили
...
var showMoreStyles = function showMoreStyles(triger, wrapper) {
  // triger - кнопка, styles - обьекты которые нужно показать
  // const cards = document.querySelectorAll(styles);   // заком. когда работаем с сервером
  var btn = document.querySelector(triger); //====================== Паказать изображения с помощью class
  // cards.forEach(item => {
  ...
  function createCards(response) {
    // response - массив, который получаем от сервера (res)
...
var sliders = function sliders(slides, dir, prev, next) {
  // dir - diraction (направление слайдера - горизонтально/вертикально)
  var slideIndex = 1,
      // индекс текущего слайда
  paused = false;
  var items = document.querySelectorAll(slides); // выбираем все слайды в слайдере

  function showSlides(n) {
    // тоже индекс слайда
    if (n > items.length) {
      // усле индекс больше чем количество слайдов в слайдере
      slideIndex = 1;
    }
  ...
  try {
    // на случай если в аргументы не переданы кнопки (авто изминение)
  ...
  items[0].parentNode.addEventListener('mouseenter', function () {
    // mouseenter == hover; 0 - только для того, чтобы получить доступ к родителю
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Could not tetch ".concat(url, ", status: ").concat(res.status));
